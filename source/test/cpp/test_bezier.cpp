...
CGLM_INLINE
float test_bezier_plain(float s, float p0, float c0, float c1, float p1)
{
...
CGLM_INLINE
float test_hermite_plain(float s, float p0, float t0, float t1, float p1)
{
    float ss, sss;
    ...
    return p0 * (2.0f * sss - 3.0f * ss + 1.0f) + t0 * (sss - 2.0f * ss + s) + p1 * (-2.0f * sss + 3.0f * ss) + t1 * (sss - ss);
}
