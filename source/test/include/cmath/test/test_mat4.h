...
{
    int  i, count;
    mat4 matrices[4] = {A_MATRIX, A_MATRIX, A_MATRIX, A_MATRIX};
...
TEST_IMPL(GLM_PREFIX, mat4_mulv)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ASSERT(test_eq(res[i], v[0] * mat[0][i] + v[1] * mat[1][i] + v[2] * mat[2][i] + v[3] * mat[3][i]))
    }
...
TEST_IMPL(GLM_PREFIX, mat4_mulv3)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ASSERT(test_eq(res[i], v[0] * mat[0][i] + v[1] * mat[1][i] + v[2] * mat[2][i] + last * mat[3][i]))
    }
...
TEST_IMPL(GLM_PREFIX, mat4_det)
{
    ...
    det1 = a * (f * t[0] - g * t[1] + h * t[2]) - b * (e * t[0] - g * t[3] + h * t[4]) + c * (e * t[1] - f * t[3] + h * t[5]) - d * (e * t[2] - f * t[4] + g * t[5]);
...
TEST_IMPL(GLM_PREFIX, mat4_rmc)
{
    ...
    for (i = 0; i < 4; i++)
    {
        v1[i] = v[0] * mat[i][0] + v[1] * mat[i][1] + v[2] * mat[i][2] + v[3] * mat[i][3];
    }
