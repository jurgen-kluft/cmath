#ifndef __CMATH_ARM_H__
#define __CMATH_ARM_H__
...
#    define glmm_xor(a, b) vreinterpretq_f32_s32(veorq_s32(vreinterpretq_s32_f32(a), vreinterpretq_s32_f32(b)))
...
{
    return vaddq_f32(vaddq_f32(glmm_splat_x(v), glmm_splat_y(v)), vaddq_f32(glmm_splat_z(v), glmm_splat_w(v)));
    /*
     this seems slower:
...
static inline float glmm_dot(float32x4_t a, float32x4_t b) { return glmm_hadd(vmulq_f32(a, b)); }
...
static inline float glmm_norm_one(float32x4_t a) { return glmm_hadd(glmm_abs(a)); }

static inline float glmm_norm_inf(float32x4_t a) { return glmm_hmax(glmm_abs(a)); }
...
static inline float32x4_t glmm_fmadd(float32x4_t a, float32x4_t b, float32x4_t c)
{
#    if CGLM_ARM64
...
static inline float32x4_t glmm_fnmadd(float32x4_t a, float32x4_t b, float32x4_t c)
{
#    if CGLM_ARM64
...
static inline float32x4_t glmm_fmsub(float32x4_t a, float32x4_t b, float32x4_t c)
{
#    if CGLM_ARM64
    return vfmsq_f32(c, a, b);
#    else
...
static inline float32x4_t glmm_fnmsub(float32x4_t a, float32x4_t b, float32x4_t c) { return vsubq_f32(vdupq_n_f32(0.0f), glmm_fmadd(a, b, c)); }

#endif
#endif /* __CMATH_ARM_H__ */
